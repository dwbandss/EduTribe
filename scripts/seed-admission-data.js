...
async function connectDB() {
  try {
    const conn = await mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/edutribe');
...
const schoolSchema = new mongoose.Schema({
  ...
  type: { type: String, enum: ['government', 'private', 'aided'], required: true },
...
const schemeSchema = new mongoose.Schema({
  name: { type: String, required: true },
  type: { type: String, enum: ['scholarship', 'admission', 'reservation', 'financial_aid'], required: true },
  category: { type: String, enum: ['general', 'sc', 'st', 'obc', 'minority', 'ews', 'other'], required: true },
  state: { type: String, required: true },
